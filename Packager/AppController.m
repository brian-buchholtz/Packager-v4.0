...
- (void)awakeFromNib {
    ...
    [self.browserPayload setCellClass:[FileSystemBrowserCell class]];
    ...
    [self.browserPayload registerForDraggedTypes:@[NSFilenamesPboardType]];
    [self.browserPayload setDraggingSourceOperationMask:NSDragOperationEvery forLocal:YES];
    [self.browserPayload setDraggingSourceOperationMask:NSDragOperationEvery forLocal:NO];
    ...
    //self.browserPayload.backgroundColor = [NSColor controlBackgroundColor];
    
    // Double click support
    self.browser.target = self;
    self.browser.doubleAction = @selector(browserDoubleClick:);
    
    self.browserPayload.target = self;
    self.browserPayload.doubleAction = @selector(browserDoubleClickPayload:);
...
- (void)OpenProject {
    ...
    if (stringProjectFile) {
        ...
        [self.browserPayload loadColumnZero];
...
- (NSInteger)browserPayload:(NSBrowser *)browserPayload numberOfChildrenOfItem:(id)item {
...
- (id)browserPayload:(NSBrowser *)browserPayload child:(NSInteger)index ofItem:(id)item {
...
- (BOOL)browserPayload:(NSBrowser *)browserPayload isLeafItem:(id)item {
...
- (id)browserPayload:(NSBrowser *)browserPayload objectValueForItem:(id)item {
...
- (void)browserPayload:(NSBrowser *)browserPayload willDisplayCell:(FileSystemBrowserCell *)cell atRow:(NSInteger)row column:(NSInteger)column {
    
    // Find the item and set the image.
    NSIndexPath *indexPath = [browserPayload indexPathForColumn:column];
    indexPath = [indexPath indexPathByAddingIndex:row];
    FileSystemNode *node = [browserPayload itemAtIndexPath:indexPath];
    cell.image = node.icon;
    cell.labelColor = node.labelColor;
...
- (NSViewController *)browserPayload:(NSBrowser *)browserPayload previewViewControllerForLeafItem:(id)item {
...
- (NSViewController *)browserPayload:(NSBrowser *)browserPayload headerViewControllerForItem:(id)item {
...
- (CGFloat)browserPayload:(NSBrowser *)browserPayload shouldSizeColumn:(NSInteger)columnIndex forUserResize:(BOOL)forUserResize toWidth:(CGFloat)suggestedWidth  {
    
    if (!forUserResize) {
        
        id item = [browserPayload parentForItemsInColumn:columnIndex];
        
        if ([self browserPayload:browserPayload isLeafItem:item]) {
...
- (BOOL)browserPayload:(NSBrowser *)browserPayload writeRowsWithIndexes:(NSIndexSet *)rowIndexes inColumn:(NSInteger)column toPasteboard:(NSPasteboard *)pasteboard {
    
    NSMutableArray *filenames = [NSMutableArray arrayWithCapacity:rowIndexes.count];
    NSIndexPath *baseIndexPath = [browserPayload indexPathForColumn:column];
    
    for (NSUInteger i = rowIndexes.firstIndex; i <= rowIndexes.lastIndex; i = [rowIndexes indexGreaterThanIndex:i]) {
        
        FileSystemNode *fileSystemNode = [browserPayload itemAtIndexPath:[baseIndexPath indexPathByAddingIndex:i]];
        [filenames addObject:(fileSystemNode.URL).path];
...
- (BOOL)browserPayload:(NSBrowser *)browserPayload canDragRowsWithIndexes:(NSIndexSet *)rowIndexes inColumn:(NSInteger)column withEvent:(NSEvent *)event {
...
- (NSImage *)browserPayload:(NSBrowser *)browserPayload draggingImageForRowsWithIndexes:(NSIndexSet *)rowIndexes inColumn:(NSInteger)column withEvent:(NSEvent *)event offset:(NSPointPointer)dragImageOffset {
    
    NSImage *result = [browserPayload draggingImageForRowsWithIndexes:rowIndexes inColumn:column withEvent:event offset:dragImageOffset];
...
- (FileSystemNode *)fileSystemNodeAtRowPayload:(NSInteger)row column:(NSInteger)column {
    
    if (column >= 0) {
        
        NSIndexPath *indexPath = [self.browserPayload indexPathForColumn:column];
        ...
        id result = [self.browserPayload itemAtIndexPath:indexPath];
        return (FileSystemNode *)result;
...
- (NSDragOperation)browserPayload:(NSBrowser *)browserPayload validateDrop:(id <NSDraggingInfo>)info proposedRow:(NSInteger *)row column:(NSInteger *)column dropOperation:(NSBrowserDropOperation *)dropOperation {
    ...
    if ([[info draggingPasteboard].types indexOfObject:NSFilenamesPboardType] > 0) {
        ...
        if (*column != -1) {
            ...
            BOOL droppingFromSameFolder = ([info draggingSource] == browserPayload) && (*column == self.draggedColumnIndex);
            
            if (*row != -1) {
                
                // If we are dropping on a folder, then we will accept the drop at that row
                FileSystemNode *fileSystemNode = [self fileSystemNodeAtRowPayload:*row column:*column];
...
- (BOOL)browserPayload:(NSBrowser *)browserPayload acceptDrop:(id <NSDraggingInfo>)info atRow:(NSInteger)row column:(NSInteger)column dropOperation:(NSBrowserDropOperation)dropOperation {
    ...
    if ((column != -1) && (filenames != nil)) {
        ...
        if (row != -1) {
            ...
            // Grab the parent for the column, which should be a directory
            targetFileSystemNode = (FileSystemNode *)[browserPayload parentForItemsInColumn:column];
    ...
    if (targetFileSystemNode != nil) {
        ...
        [warningAlert beginSheetModalForWindow:self.windowMain completionHandler:^(NSInteger result) {
            ...
            if (result == NSAlertFirstButtonReturn) {
                ...
                for (NSInteger col = self.browserPayload.lastColumn; col >= 0; col--) {
                    
                    [self.browserPayload reloadColumn:col];
...
- (void)browserDoubleClickPayload:(id)sender {
    
    // Find the clicked item and open it in Finder
    FileSystemNode *clickedNode = [self fileSystemNodeAtRowPayload:self.browserPayload.clickedRow column:self.browserPayload.clickedColumn];
